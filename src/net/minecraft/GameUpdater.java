...
public class GameUpdater implements Runnable {
	...
	public static boolean newversionUpdate = true;
	...
	protected void loadJarURLs() throws Exception {
		...
		String jarList = "lwjgl.jar, jinput.jar, lwjgl_util.jar, additional.zip, "
				+ mainGameUrl;
		...
		URL path = new URL("http://srv1.dcserver.ru:8050/update/");
				...
				.doPrivileged(new PrivilegedExceptionAction() {
					...
					public Object run() throws Exception {
						return (new StringBuilder())
								.append(Util.getWorkingDirectory())
								.append(File.separator).toString();
					}

					final GameUpdater this$0;

					{
						this$0 = GameUpdater.this;
						// super();
					}
				});
		String szZipFilePath = (new StringBuilder(String.valueOf(path)))
				.append("bin").append(File.separator).append("additional.zip")
				.toString();
		File f = new File(szZipFilePath);
		if (!f.exists())
			System.out.println((new StringBuilder("\nNot found: ")).append(
					szZipFilePath).toString());
		if (f.isDirectory())
			System.out.println((new StringBuilder("\nNot file: ")).append(
					szZipFilePath).toString());
		System.out.println("Enter path to extract files: ");
		String szExtractPath = path;
		File f1 = new File(szExtractPath);
		if (!f1.exists())
			System.out.println((new StringBuilder("\nNot found: ")).append(
					szExtractPath).toString());
		if (!f1.isDirectory())
			System.out.println((new StringBuilder("\nNot directory: ")).append(
					szExtractPath).toString());
		Vector zipEntries = new Vector();
		try {
			teststring = szExtractPath + "/buildcraft";
			File fil = new File(teststring);
			del_cat(fil);

			teststring = szExtractPath + "/config";
			File fil2 = new File(teststring);
			del_cat(fil2);

			teststring = szExtractPath + "/mods";
			File fil3 = new File(teststring);
			del_cat(fil3);

			teststring = szExtractPath + "/redpower";
			File fil4 = new File(teststring);
			del_cat(fil4);

			teststring = szExtractPath + "/resources";
			File fil5 = new File(teststring);
			del_cat(fil5);
			ZipFile zf = new ZipFile(szZipFilePath);
			for (Enumeration en = zf.entries(); en.hasMoreElements(); zipEntries
					.addElement((ZipEntry) en.nextElement()))
				;
	...
	public static void del_cat(File file) {
		if (!file.exists())
			return;
		if (file.isDirectory()) {
	...
	static void extractFromZip(String szZipFilePath, String szExtractPath,
		...
		char szDstName[] = new char[szName.length()];
		for (int i = 0; i < szName.length(); i++)
			if (szName.charAt(i) == '/')
				szDstName[i] = File.separatorChar;
			else
				szDstName[i] = szName.charAt(i);

		String dst = new String(szDstName);

		// String szDstName = slash2sep(szName);
		String szEntryDir;
		if (dst.lastIndexOf(File.separator) != -1)
			szEntryDir = dst.substring(0, dst.lastIndexOf(File.separator));
		else
			szEntryDir = "";
		System.out.print(szDstName);
		long nSize = ze.getSize();
		long nCompressedSize = ze.getCompressedSize();
		System.out.println((new StringBuilder(" ")).append(nSize).append(" (")
				.append(nCompressedSize).append(")").toString());
		try {
			File newDir = new File((new StringBuilder(
					String.valueOf(szExtractPath))).append(File.separator)
					.append(szEntryDir).toString());
			newDir.mkdirs();
			FileOutputStream fos = new FileOutputStream((new StringBuilder(
					String.valueOf(szExtractPath))).append(File.separator)
					.append(szDstName).toString());
