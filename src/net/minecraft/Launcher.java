...
public class Launcher extends Applet implements Runnable, AppletStub,
	...
	public void init(String userName, String latestVersion,
		...
		try {
			bgImage = ImageIO.read(LoginForm.class.getResource("dirt.png"))
		...
		gameUpdater = new GameUpdater(latestVersion, "minecraft.jar?user="
	...
	public void paint(Graphics g2) {
		...
		if (gameUpdater.pauseAskUpdate) {
			...
			if (!gameUpdater.fatalError) {
				...
				g.fillRect(64, h - 64,
						gameUpdater.percentage * (w - 128) / 100, 4);
				g.setColor(new Color(2138144));
				g.fillRect(65, h - 64 + 1, gameUpdater.percentage * (w - 128)
						/ 100 - 2, 1);
			}
		}
	...
	public URL getDocumentBase() {
		try {
			return new URL("http://dcserver.ru");
